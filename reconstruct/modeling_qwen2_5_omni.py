...
class Qwen2_5OmniThinkerCausalLMOutputWithPast(ModelOutput):
    ...
    masked_token_positions: Optional[torch.BoolTensor] = None
    modality_positions: Optional[dict] = None  # {'text': List[torch.LongTensor], 'audio': List[torch.LongTensor], 'video': List[torch.LongTensor]}
...
class Qwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def _generate_span_mask(
        ...
        if seq_len <= 0 or num_to_mask <= 0:
            return torch.zeros(seq_len, dtype=torch.bool, device=device)

        if num_to_mask >= seq_len:
            return torch.ones(seq_len, dtype=torch.bool, device=device)

        min_span = max(1, int(min_span))
        max_span = max(min_span, int(max_span))
        max_span = min(max_span, seq_len)

        avg_span = (min_span + max_span) / 2.0
        num_spans = max(1, int(math.ceil(num_to_mask / avg_span)))
        max_start = max(1, seq_len - min_span + 1)
        num_spans = min(num_spans, max_start)

        starts = torch.randperm(max_start, device=device)[:num_spans]
        starts = starts.sort()[0]

        lengths = torch.randint(low=min_span, high=max_span + 1, size=(num_spans,), device=device)
        max_lengths = seq_len - starts
        ...
        if starts.numel() == 0:
            ...
            selected = torch.randperm(seq_len, device=device)[:num_to_mask]
            mask[selected] = True
            return mask

        ends = starts + lengths

        diff = torch.zeros(seq_len + 1, dtype=torch.int32, device=device)
        ones = torch.ones_like(starts, dtype=torch.int32)
        diff.index_add_(0, starts, ones)
        diff.index_add_(0, ends, -ones)

        mask = diff.cumsum(0)[:-1] > 0

        current = int(mask.sum().item())
        if current > num_to_mask:
            indices = torch.nonzero(mask, as_tuple=False).squeeze(-1)
            keep = indices[torch.randperm(indices.numel(), device=device)[:num_to_mask]]
            new_mask = torch.zeros_like(mask)
            new_mask[keep] = True
            mask = new_mask
        elif current < num_to_mask:
            remaining = torch.nonzero(~mask, as_tuple=False).squeeze(-1)
            if remaining.numel() > 0:
                extra = remaining[torch.randperm(remaining.numel(), device=device)[: num_to_mask - current]]
                mask[extra] = True

        return mask

    def _span_mask_for_vision(
        self,
        seq_len: int,
        mask_ratio: float,
        min_span: int = 0.05,
        ...
        num_groups_total = math.ceil(seq_len / spatial_merge_unit)

        if mask_ratio <= 0:
            token_mask = torch.zeros(seq_len, dtype=torch.bool, device=device)
            group_mask = torch.zeros(num_groups_total, dtype=torch.bool, device=device)
            return token_mask, group_mask

        if mask_ratio >= 1:
            token_mask = torch.ones(seq_len, dtype=torch.bool, device=device)
            group_mask = torch.ones(num_groups_total, dtype=torch.bool, device=device)
            return token_mask, group_mask

        num_groups = num_groups_total
        total_tokens_to_mask = int(round(seq_len * mask_ratio))
        groups_to_mask = math.ceil(total_tokens_to_mask / spatial_merge_unit)
        groups_to_mask = min(groups_to_mask, num_groups)

        group_mask = torch.zeros(num_groups, dtype=torch.bool, device=device)
        if groups_to_mask > 0:
            group_mask[-groups_to_mask:] = True

        token_mask = group_mask.repeat_interleave(spatial_merge_unit)
        if token_mask.size(0) > seq_len:
            token_mask = token_mask[-seq_len:]
        elif token_mask.size(0) < seq_len:
            pad_len = seq_len - token_mask.size(0)
            token_mask = torch.cat(
                [torch.zeros(pad_len, dtype=torch.bool, device=device), token_mask], dim=0
            )
    ...
    def forward(
        ...
        Args:
            ...
            mask_ratio (`float`, *optional*):
                The ratio of patches to mask. If provided, patches will be masked before Transformer blocks.
            mask_token (`torch.Tensor`, *optional*):
                The learnable mask token to replace masked patches.
        ...
        vision_mask_positions_list = []
        if mask_ratio is not None and mask_ratio > 0 and mask_token is not None:
            masked_hidden_states_list = []
            start_idx = 0
            for idx in range(len(grid_thw)):
                T, H, W = grid_thw[idx]
                llm_grid_h = H // self.spatial_merge_size
                llm_grid_w = W // self.spatial_merge_size
                num_tokens = T * llm_grid_h * llm_grid_w * self.spatial_merge_unit
                end_idx = start_idx + num_tokens

                token_embeds = hidden_states[start_idx:end_idx]
                token_seq_len = token_embeds.shape[0]

                mask_indices, group_mask = self._span_mask_for_vision(token_seq_len, mask_ratio, device=token_embeds.device)

                if mask_indices.any():
                    masked_token_embeds = token_embeds.clone()
                    mask_tokens = mask_token.expand(mask_indices.sum().item(), -1).to(token_embeds.device, token_embeds.dtype)
                    masked_token_embeds[mask_indices] = mask_tokens
                    masked_hidden_states_list.append(masked_token_embeds)
                else:
                    masked_hidden_states_list.append(token_embeds)
                
                vision_mask_positions_list.append(group_mask)

                start_idx = end_idx

            if masked_hidden_states_list:
                masked_hidden_states = torch.cat(masked_hidden_states_list, dim=0)
                hidden_states_list = [hidden_states, masked_hidden_states]

        base_rotary_pos_emb = self.rot_pos_emb(grid_thw)
        window_index, cu_window_seqlens = self.get_window_index(grid_thw)

        window_index = window_index.to(hidden_states.device)
        base_rotary_pos_emb = base_rotary_pos_emb.to(hidden_states.device)
        ...
        def encode_single(sequence: torch.Tensor) -> torch.Tensor:
            local_seq_len, _ = sequence.size()
            reshaped_states = sequence.reshape(
                local_seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1
            )
            reshaped_states = reshaped_states[window_index, :, :]
            reshaped_states = reshaped_states.reshape(local_seq_len, -1)

            rotary = base_rotary_pos_emb.reshape(
                local_seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1
            )
            rotary = rotary[window_index, :, :]
            rotary = rotary.reshape(local_seq_len, -1)

            hidden = reshaped_states
            for layer_num, blk in enumerate(self.blocks):
                ...
                if self.gradient_checkpointing and self.training:
                    hidden = self._gradient_checkpointing_func(
                        blk.__call__, hidden, cu_seqlens_now, rotary
                    )
                else:
                    hidden = blk(hidden, cu_seqlens=cu_seqlens_now, rotary_pos_emb=rotary)
        ...
        for idx, sequence in enumerate(hidden_states_list):
            if len(hidden_states_list) == 2 and idx == 0:
                with torch.no_grad():
                    encoded = encode_single(sequence)
                encoded = encoded.detach()
            else:
                encoded = encode_single(sequence)
            return_hidden_states_list.append(encoded)

        if mask_ratio is not None and mask_ratio > 0 and mask_token is not None and vision_mask_positions_list:
            return return_hidden_states_list, vision_mask_positions_list
        else:
            return return_hidden_states_list
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        use_partial_layers = layers is not None and layers >= 0
        target_layers = min(layers, total_layers) if use_partial_layers else total_layers
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.mask_text_token = nn.Parameter(torch.randn(1, 1, text_hidden_size, dtype=torch.float32) * 0.02)
        self.mask_audio_token = nn.Parameter(torch.randn(1, 1, text_hidden_size, dtype=torch.float32) * 0.02)
        self.mask_video_token = nn.Parameter(torch.randn(1, vision_hidden_size, dtype=torch.float32) * 0.02)
    ...
    def _span_mask(self, seq_len: int, mask_ratio: float, min_span: int = 0.1, max_span: int =0.3, device=None, random_mask: bool = False):
        """
    ...
    def forward(
        ...
        mask_text:Optional[bool] = False,
        mask_audio:Optional[bool] = False,
        mask_video:Optional[bool] = False,
        mask_text_ratio:Optional[float] = 0.6,
        mask_audio_ratio:Optional[float] = 0.3,
        mask_video_ratio:Optional[float] = 0.3,
        random_mask:Optional[bool] = False,
        title_token_indices: Optional[torch.LongTensor] = None,
        layers: int = -1,
        simple_classify: bool = False,
        ...
        inputs_embeds_unmasked = inputs_embeds.clone()
        inputs_embeds_masked = inputs_embeds.clone()
        has_masked_sequence = False
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            ...
            text_token_mask = (
                ...
                (input_ids != self.config.audio_start_token_id) &
                (input_ids != self.config.audio_end_token_id)
            )
            
            for batch_idx in range(input_ids.shape[0]):
                text_positions = torch.where(text_token_mask[batch_idx])[0]
                modality_positions['text'].append(text_positions)
            
            if mask_text and mask_text_ratio > 0:
                for batch_idx in range(input_ids.shape[0]):
                    text_positions = modality_positions['text'][batch_idx]
                    if len(text_positions) > 0:
                        text_seq_len = len(text_positions)
                        mask_indices = self._span_mask(text_seq_len, mask_text_ratio, device=input_ids.device, random_mask=random_mask)
                        masked_positions = text_positions[mask_indices]
                        
                        if len(masked_positions) > 0:
                            mask_token = (
                                self.mask_text_token[0, 0]
                                .expand(len(masked_positions), -1)
                                .to(inputs_embeds.device, inputs_embeds.dtype)
                            )
                            inputs_embeds_masked[batch_idx, masked_positions] = mask_token
                            has_masked_sequence = True
                            if batch_idx >= len(all_masked_positions):
                                all_masked_positions.append(torch.zeros(input_ids.shape[1], dtype=torch.bool, device=input_ids.device))
                            all_masked_positions[batch_idx][masked_positions] = True
            ...
            if input_features is not None:
                ...
                audio_features_vanilla = audio_outputs.last_hidden_state

                if audio_features_vanilla.shape[0] != sum(audio_output_lengths.tolist()):
                ...
                for batch_idx in range(input_ids.shape[0]):
                    audio_positions = torch.where(input_ids[batch_idx] == self.config.audio_token_id)[0]
                    modality_positions['audio'].append(audio_positions)

                audio_features_vanilla = audio_features_vanilla.to(inputs_embeds.device, inputs_embeds.dtype)
                audio_features_masked = audio_features_vanilla

                if mask_audio and mask_audio_ratio > 0:
                    audio_features_list = audio_features_vanilla.split(audio_output_lengths.tolist(), dim=0)
                    masked_audio_features_list = []
                    for batch_idx, audio_feat in enumerate(audio_features_list):
                        audio_seq_len = audio_feat.shape[0]
                        mask_indices = self._span_mask(audio_seq_len, mask_audio_ratio, device=audio_feat.device, random_mask=random_mask)
                        masked_audio_feat = audio_feat.clone()
                        if mask_indices.any():
                            mask_token = (
                                self.mask_audio_token[0, 0]
                                .expand(mask_indices.sum().item(), -1)
                                .to(audio_feat.device, audio_feat.dtype)
                            )
                            masked_audio_feat[mask_indices] = mask_token
                            
                            if batch_idx >= len(all_masked_positions):
                                all_masked_positions.append(torch.zeros(input_ids.shape[1], dtype=torch.bool, device=input_ids.device))
                            audio_token_positions = torch.where(input_ids[batch_idx] == self.config.audio_token_id)[0]
                            if len(audio_token_positions) >= len(mask_indices):
                                masked_audio_positions = audio_token_positions[mask_indices]
                                all_masked_positions[batch_idx][masked_audio_positions] = True
                        masked_audio_features_list.append(masked_audio_feat)
                    audio_features_masked = torch.cat(masked_audio_features_list, dim=0)
                    has_masked_sequence = True

                inputs_embeds_unmasked = inputs_embeds_unmasked.masked_scatter(audio_mask, audio_features_vanilla)
                inputs_embeds_masked = inputs_embeds_masked.masked_scatter(audio_mask, audio_features_masked)

            if pixel_values is not None:
                pixel_values = pixel_values.type(self.visual.dtype)

                mask_ratio = mask_video_ratio if (mask_video and mask_video_ratio > 0) else None
                mask_token_param = self.mask_video_token if mask_ratio is not None else None

                image_embeds_output = self.visual(
                    pixel_values, 
                    grid_thw=image_grid_thw,
                    mask_ratio=mask_ratio,
                    mask_token=mask_token_param
                )

                if isinstance(image_embeds_output, tuple) :
                    image_embeds_list, image_mask_positions_list = image_embeds_output
                    image_embeds_vanilla = image_embeds_list[0].to(inputs_embeds.device, inputs_embeds.dtype)
                    image_embeds_masked = image_embeds_list[-1].to(inputs_embeds.device, inputs_embeds.dtype)
                    has_masked_sequence = True
                else:
                    image_embeds_vanilla = image_embeds_output.to(inputs_embeds.device, inputs_embeds.dtype)
                    image_embeds_masked = image_embeds_vanilla.clone()
                    image_mask_positions_list = None
                ...
                inputs_embeds_unmasked = inputs_embeds_unmasked.masked_scatter(image_mask, image_embeds_vanilla)
                inputs_embeds_masked = inputs_embeds_masked.masked_scatter(image_mask, image_embeds_masked)
                
                # if image_mask_positions_list is not None:
                #     for batch_idx in range(input_ids.shape[0]):
                #         if batch_idx >= len(all_masked_positions):
                #             all_masked_positions.append(torch.zeros(input_ids.shape[1], dtype=torch.bool, device=input_ids.device))
                #         image_token_positions = torch.where(input_ids[batch_idx] == self.config.image_token_id)[0]
                #         for sample_idx, mask_indices in enumerate(image_mask_positions_list):
                #             num_masked = mask_indices.sum().item()
                #             if num_masked > 0:
                #                 masked_indices_in_feat = torch.where(mask_indices)[0]
                #                 valid_global_indices = image_token_idx + masked_indices_in_feat
                #                 valid_global_indices = valid_global_indices[valid_global_indices < len(image_token_positions)]
                #                 if len(valid_global_indices) > 0:
                #                     masked_image_positions = image_token_positions[valid_global_indices]
                #                     all_masked_positions[batch_idx][masked_image_positions] = True
                #             image_token_idx += len(mask_indices)
            ...
            if pixel_values_videos is not None:
                ...
                video_embeds_output = self.visual(
                    pixel_values_videos, 
                ...
                if isinstance(video_embeds_output, tuple) :
                    video_embeds_list, video_mask_positions_list = video_embeds_output
                    video_embeds_vanilla = video_embeds_list[0].to(inputs_embeds.device, inputs_embeds.dtype)
                    video_embeds_masked = video_embeds_list[-1].to(inputs_embeds.device, inputs_embeds.dtype)
                    has_masked_sequence = True
                else:
                    video_embeds_vanilla = video_embeds_output[0].to(inputs_embeds.device, inputs_embeds.dtype)
                    video_embeds_masked = video_embeds_vanilla.clone()
                    video_mask_positions_list = None
                ...
                for batch_idx in range(input_ids.shape[0]):
                    video_positions = torch.where(input_ids[batch_idx] == self.config.video_token_id)[0]
                    modality_positions['video'].append(video_positions)

                inputs_embeds_unmasked = inputs_embeds_unmasked.masked_scatter(video_mask, video_embeds_vanilla)
                inputs_embeds_masked = inputs_embeds_masked.masked_scatter(video_mask, video_embeds_masked)
                
                if video_mask_positions_list is not None:
                    for batch_idx in range(input_ids.shape[0]):
                        if batch_idx >= len(all_masked_positions):
                            all_masked_positions.append(torch.zeros(input_ids.shape[1], dtype=torch.bool, device=input_ids.device))
                        
                        vision_mask_positions = torch.cat(video_mask_positions_list, dim=0)
                        
                        video_mask_batch = video_mask[batch_idx, :, 0]  # shape: (seq_len,)
                        video_token_positions = torch.where(video_mask_batch)[0]
                        
                        masked_indices_in_vision = torch.where(vision_mask_positions)[0]
                        masked_positions_in_multimodal = video_token_positions[0] + masked_indices_in_vision
                        all_masked_positions[batch_idx][masked_positions_in_multimodal] = True 
            if all_masked_positions:
                masked_token_positions = torch.stack(all_masked_positions, dim=0)
            else:
                masked_token_positions = torch.zeros(input_ids.shape, dtype=torch.bool, device=input_ids.device) if input_ids is not None else None

            if attention_mask is not None:
                attention_mask = attention_mask.to(inputs_embeds_masked.device)
            
        else:
            masked_token_positions = None

        use_partial_layers = layers is not None and layers >= 0
        model_layers = layers if use_partial_layers else None
        ...
        if use_partial_layers:
            ...
            if simple_classify:
                outputs_unmasked = self.model(
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                    past_key_values=past_key_values,
                    inputs_embeds=inputs_embeds_unmasked,
                    use_cache=use_cache,
                    output_attentions=output_attentions,
                    output_hidden_states=output_hidden_states,
                    return_dict=return_dict,
            ...
            hidden_states_unmasked = outputs_unmasked
            hidden_states_masked = outputs_masked
            return hidden_states_unmasked, hidden_states_masked, masked_token_positions, modality_positions
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    @classmethod
    def _mask_tokens_path(cls, model_path):
        import os

        root = os.environ.get("PCDD_MASK_TOKEN_DIR", "checkpoint/reconstruct/mask_tokens")
        model_name = os.path.basename(os.path.abspath(str(model_path))) or "qwen2_5_omni"
        path = os.path.join(root, model_name)
        os.makedirs(path, exist_ok=True)
        return os.path.join(path, "mask_tokens.pt")

    @classmethod
    def _initialize_and_save_mask_tokens(cls, model_path, config):
        text_hidden_size = config.thinker_config.text_config.hidden_size
        vision_hidden_size = config.thinker_config.vision_config.hidden_size

        mask_tokens = {
            'mask_text_token': torch.randn(1, 1, text_hidden_size, dtype=torch.float32) * 0.02,
            'mask_audio_token': torch.randn(1, 1, text_hidden_size, dtype=torch.float32) * 0.02,
            'mask_video_token': torch.randn(1, vision_hidden_size, dtype=torch.float32) * 0.02,
        }

        mask_tokens_path = cls._mask_tokens_path(model_path)
        torch.save(mask_tokens, mask_tokens_path)
        print(f"Initialized and saved mask tokens to {mask_tokens_path}")
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model_path = pretrained_model_name_or_path if os.path.isdir(pretrained_model_name_or_path) else getattr(model.config, '_name_or_path', pretrained_model_name_or_path)
        mask_tokens_path = cls._mask_tokens_path(model_path)

        if os.path.exists(mask_tokens_path):
            try:
                os.remove(mask_tokens_path)
                print(f"Removed existing mask tokens file: {mask_tokens_path}")
            except Exception as e:
                print(f"Warning: Failed to remove existing mask tokens file: {e}")

        mask_tokens = cls._initialize_and_save_mask_tokens(model_path, model.config)

        with torch.no_grad():
            for token_name in ['mask_text_token', 'mask_audio_token', 'mask_video_token']:
                mask_token = getattr(model.thinker, token_name, None)
                if mask_token is not None and not mask_token.is_meta and token_name in mask_tokens:
                    mask_token.data.copy_(mask_tokens[token_name].to(dtype=mask_token.dtype, device=mask_token.device))
    ...
    def generate(
        ...
        **kwargs,
        ...
        use_partial_layers = layers is not None and layers >= 0
        if use_partial_layers:
            thinker_kwargs.pop("max_new_tokens", None)
            thinker_result = self.thinker(input_ids=input_ids, layers=layers, **thinker_kwargs)
            outputs_unmasked, outputs_masked, masked_token_positions, modality_positions = thinker_result
            return outputs_unmasked, outputs_masked, masked_token_positions, modality_positions
        else:
